import PropTypes from 'prop-types';
import '../../styles/SimpleCounter.css';
const SimpleCounter = (props) => {
    return (
        <div className="container d-flex justify-content-center align-items-center vh-100">
            <div className="row counter-display align-items-center">
                <div className="col-auto clock-img">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" className="bi bi-clock" viewBox="0 0 16 16">
                        <path d="M8 3.5a.5.5 0 0 0-1 0V9a.5.5 0 0 0 .252.434l3.5 2a.5.5 0 0 0 .496-.868L8 8.71z" />
                        <path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16m7-8A7 7 0 1 1 1 8a7 7 0 0 1 14 0" />
                    </svg>
                </div>
                <div className="col seconds mx-1">{props.hundredThousandsDigit}</div>
                <div className="col seconds mx-1">{props.tenThousandsDigit}</div>
                <div className="col seconds mx-1">{props.thousandsDigit}</div>
                <div className="col seconds mx-1">{props.hundredsDigit}</div>
                <div className="col seconds mx-1">{props.tensDigit}</div>
                <div className="col seconds mx-1">{props.onesDigit}</div>
            </div>
        </div>

    )
}

SimpleCounter.propTypes = {
    hundredThousandsDigit: PropTypes.number,
    tenThousandsDigit: PropTypes.number,
    thousandsDigit: PropTypes.number,
    hundredsDigit: PropTypes.number,
    tensDigit: PropTypes.number,
    onesDigit: PropTypes.number,
}

export default SimpleCounter;